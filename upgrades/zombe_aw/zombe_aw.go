...
func (w *Worker) Setup() {
	...
	w.iconRec = rl.NewRectangle(240, 0, float32(w.tex.Width/10), float32(w.tex.Height/10))
	w.ShowText = false
}
func (w *Worker) Draw() {
	rl.DrawTexture(*texture, int32(w.x), int32(w.y), rl.White)
	rl.DrawTextureRec(*w.tex, w.iconRec, rl.NewVector2(w.x+8, w.y+13), rl.White)
	rl.DrawText("Zombe Adult Workers", int32(w.x+100), int32(w.y+10), 24, rl.White)
	...
	if w.ShowText {
		rl.DrawText("Zombe Adult Worker", 30, 120, 30, rl.LightGray)
		rl.DrawText("This is a very pricy of a worker,\nbut allways work 24H a day", 30, 160, 20, rl.LightGray)
